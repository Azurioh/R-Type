...
void Network::Client::HandleReadHeader(const boost::system::error_code& ec, std::size_t bytes)
{
    if (!ec && bytes == HEADER_SIZE) {
        ...
        if (_currentLength > 0 && _currentLength <= MAX_MESSAGE_SIZE) {
            ...
            if (_onData) {
                std::vector<std::uint8_t> fullRaw(_headerBuffer.begin(), _headerBuffer.end());
                _onData(_id, fullRaw);
            }
            StartReadHeader();
        } else {
            Misc::Utils::Log(std::format("Client {} sent invalid content length: {}", _id, _currentLength), Misc::Utils::LogLevel::Warning);
            if (_onDisconnect) {
                _onDisconnect(_id);
            }
        }
        ...
        if (ec != boost::asio::error::operation_aborted && ec != boost::asio::error::eof && ec != boost::asio::error::connection_reset) {
            Misc::Utils::Log(std::format("Client {} header read error: {}", _id, ec.message()), Misc::Utils::LogLevel::Warning);
        }
...
void Network::Client::HandleReadContent(const boost::system::error_code& ec, std::size_t bytes)
{
    if (!ec && bytes == _currentLength) {
        if (_onData) {
            std::vector<std::uint8_t> fullRaw;
            fullRaw.reserve(_headerBuffer.size() + _contentBuffer.size());
            fullRaw.insert(fullRaw.end(), _headerBuffer.begin(), _headerBuffer.end());
            fullRaw.insert(fullRaw.end(), _contentBuffer.begin(), _contentBuffer.end());
            _onData(_id, fullRaw);
        }
        StartReadHeader();
    } else {
        if (ec != boost::asio::error::operation_aborted && ec != boost::asio::error::eof && ec != boost::asio::error::connection_reset) {
            Misc::Utils::Log(std::format("Client {} content read error: {}", _id, ec.message()), Misc::Utils::LogLevel::Warning);
        }
        if (_onDisconnect) {
            _onDisconnect(_id);
        }
